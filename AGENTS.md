...
### Since-main default review view and the unified review navigator

The default code-review diff is **`since-base`** — a composite of `merge-base(base, HEAD)` vs the working tree plus untracked files ("everything a PR would show if you committed and pushed now").

The review's left panel is ONE surface, `ReviewNavigator` (#1524), carrying two **independent** segmented controls under the existing worktree / diff-type / base-reference pickers: **File layout** (`Flat` | `Tree`) and **Grouping** (`All` | `By Git status`). All four combinations render, changing one never changes the other, and both are persisted cookie-only (`reviewNavigatorLayout`, `reviewNavigatorGrouping`). It replaced the retired three-way `Tree | Git status | Commits` toggle (`PanelViewToggle`, `SectionsPanel`, `CommitsPanel`, `resolvePanelView`, `useCommitsView`, the `reviewPanelView` / `reviewPanelViewLastUsed` pair and the panel-pair self-heal are all gone).

**Layout** is `NavigatorCollection`: `Flat` prints the filename first with the directory as quiet secondary text, `Tree` nests expandable directories (single-child folder chains merged, and a folder whose only child is a file suppresses its totals as redundant). Folders are **expanded unless explicitly collapsed**, so a refreshed diff that introduces a directory needs no re-seeding pass. Every file collection goes through it — the combined All set, Staged, Unstaged, and the files inside a commit — so the layout applies uniformly.

**Grouping = All** lists the ACTIVE diff's files, no sections. **Grouping = By Git status** renders three collapsible sections with aggregate `+/-` on each header: **Staged** (the index), **Unstaged** (working-tree changes not staged, with untracked files folded in and still carrying their `U` letter), and **Committed** — one expandable row per commit in the review range (`NavigatorCommitSection`), newest first, over the same first-parent walk and `In <base>` boundary the old rail had, with the HEAD badge on the subject line, the author + avatar + relative time + short sha on a quiet second line, and `Show more` paging at the end. Expanding a commit lazily fetches `GET /api/commit-files?sha=` (cached for the session; a commit's diff is immutable) and lists what THAT commit changed. There are no explicit file or commit counts anywhere in the list; the viewed `N/M` counter lives in the fixed toolbar, outside it.

**Selection carries a scope**, `NavigatorSelection = { scope: {kind:'working'} | {kind:'commit',sha}, path }` — there is no mode to enter or leave. Picking a file inside a commit row switches the diff to `commit:<sha>` (vs its first parent) and focuses that file; picking any Staged/Unstaged/All row restores the session's WORKING diff from `workingSet`, the snapshot App takes on every applied non-commit diff (files + sections + diff type + base). That snapshot is also what keeps Staged/Unstaged describing the working tree while a commit diff is on screen. A path with both committed and uncommitted work legitimately appears twice — once under Unstaged with its working-tree delta, once inside each commit with that commit's delta — so the highlight and the viewed mark key on the PAIR: `viewedFiles` is the working scope's (draft-persisted, unchanged), and `commitViewedFiles` is a session-only `Map<sha, Set<path>>` for commit rows. The load-time snap-back survives: a page served a `commit:<sha>` diff has no working snapshot, so it snaps once to the session default (`resolveCommitExitDiff`).

**Grouping availability is honest, and never rewrites a setting.** `resolveGroupingCapability` disables the `By Git status` segment with a stated reason for sessions that have no git index the review server reads — PR reviews (no working tree), multi-repo workspaces (no single index), and jj / GitButler / Perforce. `resolveEffectiveGrouping` additionally degrades to `All` **in the render**, with the reason on screen above the list, when the active diff has no git-status partition (the `sections` sidecar exists only for the composite `since-base` view). The SELECTION survives that fallback, so returning to a partitionable diff restores the sections without re-picking. The one remaining diff coupling runs the other way and is session-only: clicking `By Git status` while a classic diff is active switches the LIVE diff to `since-base` (what the retired Git status segment did) and persists no diff default. Nothing is coupled to `defaultDiffType` any more — `setReviewDefaultDiffType` is a plain write, and the old `sections ⟺ since-base` pair (with its `configStore.init()` split-brain and the App self-heal that repaired it) no longer exists.

**Settings migration.** Both registry entries migrate the retired cookies on READ and never write them back — `reviewPanelViewLastUsed` first, then `reviewPanelView`, mapped by `navigatorPairFromLegacyPanelView` in `packages/ui/utils/reviewNavigator.ts` (deliberately NOT in `@plannotator/core`: `@plannotator/ui` pins the published core exactly and may only import subpaths that version exports, which `packages/ui/scripts/package-smoke.ts` enforces) (`tree` → Tree + All, `sections` → Flat + By Git status; the session-only `commits` has no mapping). A migrating read returns a value, so the registry's default-seeding write never fires and resolution stays pure and identical every load until the reviewer touches a control — the same shape as the `tokenHoverCards` → `tokenHoverTrigger` migration. `hasPersistedNavigatorChoice()` counts a legacy cookie as a real choice, so first-run seeding never resets an upgrading reviewer. A review OPENS on caller-pinned flags (`plannotator review --base <ref>` / `--diff-type <id>`, session-only: `openStatePinned` on `/api/diff` skips the first-run dialog WITHOUT consuming its cookie and never persists anything) ?? the persisted pair ?? the built-in default. `ReviewSetupDialog` (first-run, reopenable from the review header menu) offers the same two controls plus the default diff, seeds an unseen reviewer to **Tree + All** while preserving whichever `defaultDiffType` the store resolved, and marks review-setup-seen when it seeds — not only on dismiss.
...
| `/api/commits`        | GET    | One page of the branch's linear `--first-parent` history for the navigator's Committed section (`?limit=&before=`) → `{ commits, hasMore, base }`. Rows carry `isHead` / `isPastBase` (where the branch meets the active base), their own `additions` / `deletions` (summed from a `--numstat` pass that rides the same `git log`, so a COLLAPSED commit row needs no expansion; binary-only commits legitimately read `+0 -0`) and best-effort author `avatarUrl`. Plain local git sessions only (PR/workspace/GitButler/jj/p4 → 400); computed against the active diff's cwd, so worktree sessions list the worktree's history. |
| `/api/commit-files`   | GET    | The files ONE commit changed, with per-file `+/-` (`?sha=<bare hex>`) → `{ sha, files: [{ path, oldPath?, status, additions, deletions, binary }] }`. Fetched lazily when the navigator expands a commit row and cached for the session (a commit's diff is immutable), so a review that never expands one never runs these diffs. Uses the same ref pair `runGitDiff` uses for `commit:<sha>` (first parent, or the empty tree for a root commit), so the list can never disagree with the patch a click on one of those rows renders. Same session gate as `/api/commits`; `400` there, for a sha that is not a bare hex object id, and for one that does not resolve. Shared logic in `packages/shared/commit-history.ts` (`listCommitFiles`, plus the pure `parseCommitLogWithNumstat` / `zipCommitFileEntries`), vendored to Pi. |
